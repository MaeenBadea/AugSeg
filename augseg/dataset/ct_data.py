...
class ct_dset(BaseDataset):
    def __init__(
        self, data_root, data_list, trs_form, trs_form_strong=None, 
        seed=0, n_sup=14460, split="val", flag_semi=False,
        mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
    ):
        super(ct_dset, self).__init__(data_list)
    ...
    def combine_channels(self, mask):
        ...
        combined_mask = np.zeros(lung.shape, dtype=np.uint8)
        # remove noise
        combined_mask[lung > 80] = 1
        combined_mask[heart > 80] = 2
        combined_mask[trachea > 80] = 3
...
def build_ct_loader(split, all_cfg, seed=0):
    ...
    # create dataset
    dset = ct_dset(cfg["data_root"], cfg["data_list"], trs_form, None, 
...
def build_ct_semi_loader(split, all_cfg, seed=0):
    split = "train"
    ...
    n_sup = 14460 - cfg.get("n_sup", 14460) # oversample labeled data to the amount of unlabeled data
    ...
    dset = ct_dset(cfg["data_root"], cfg["data_list"], trs_form_weak, None, 
    ...
    dset_unsup = ct_dset(cfg["data_root"], data_list_unsup, trs_form_weak, trs_form_strong,
                            seed, n_sup, split,
                            flag_semi=True,
                            mean=mean, std=std)
